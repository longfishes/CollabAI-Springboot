package com.longfish.collabai.constant;

public interface RabbitMQConstant {

    String EMAIL_QUEUE = "collabai.email.queue";

    String EMAIL_EXCHANGE = "collabai.email.exchange";

    String PHONE_QUEUE = "collabai.phone.queue";

    String PHONE_EXCHANGE = "collabai.phone.exchange";

    String SUMMARIZE_QUEUE = "collabai.summarize.queue";

    String SUMMARIZE_EXCHANGE = "collabai.summarize.exchange";

    String RECOGNIZE_QUEUE = "collabai.recognize.queue";

    String RECOGNIZE_EXCHANGE = "collabai.recognize.exchange";
}
